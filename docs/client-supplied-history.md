...
| Queued transports: several near-cap bodies for one user    | The callback/SSE queue is persisted as one storage value. A request that would push the serialized queue past 1.5 MiB gets 429 `QUEUE_BYTES_EXCEEDED` with `Retry-After: 5`, like depth. The entry currently being processed stays counted against the budget until its turn ends, so a transient-failure retry always fits; if a retry ever no longer fits, it fails permanently rather than exceeding the storage limit. Retry after the queue drains. |
